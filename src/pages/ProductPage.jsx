...
const ProductPage = () => {
    ...
    return (
        <div className="max-xl:pt-16 bg-primary-bg flex flex-col  min-h-screen p-3 xl:p-6">
                ...
                /> :
                    <>
                        ...
                        <div className="flex flex-col md:flex-row gap-2 justify-between md:items-center xl:mt-24 mb-3 mt-2">
                            <div className='text-primary-text text-3xl'><span className='text-xs xl:text-base'>Search results for : </span>{filterSearch || subcat || cat || type || whatsnew || ''}</div>
                            ...
                            error ? <ErrorComponent /> : <>
                                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 3xl:grid-col-8 4xl:grid-cols-9 gap-3 xl:gap-6">
                                    {filteredProducts.length > 0 && filteredProducts?.map((product, index) => (
                                        ...
                                        >
                                            {product?.fields?.Available == 'No' &&
                                                <div className='absolute top-2 right-2 cursor-pointer w-fit h-fit py-1 px-2 border border-primary-border rounded-lg ml-auto text-base z-[1] bg-primary-bg'>Out Of Stock</div>
                                            }
